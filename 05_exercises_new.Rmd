...
small_trains %>% 
  mutate(je_suis_desolee = num_arriving_late/total_num_trips) %>%
  filter(service == "International") %>% 
  ggplot(aes(y = fct_reorder(departure_station, je_suis_desolee))) +
  geom_bar() + 
  labs(title = "International Train Service in France",
      y = "",
      x = "Total Number of Trips from Each Station") 
...
```{r, fig.alt= "Mallorca Map chat! add something fun"}
...
```{r, fig.alt= "The Heather description"}
ggmap(panama_map) +
...
## COVID-19 data
  ...
  * Use `group = date` in `aes()`. #what does this mean?  
...
```{r, fig.alt="write something here"}
covid19 %>% 
  filter(day(date) == 15) %>% 
  mutate(state = str_to_lower(state)) %>% 
  left_join(census_pop_est_2018,
